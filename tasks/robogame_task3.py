...
class RoboGameTask3(RLTask):
    ...
    def get_robogame(self):
        self.robogame = RoboGame(prim_path=self.default_zero_env_path, single_robot=self.pathtest)
        for i in range(self._totnum_agents):
    ...
    def get_observations(self) -> dict:
        ...
        # for i in range(self._totnum_agents):
        #     self.obs_buf[:, 3 * i:3 * i + 3] = self.robot_translation[i, ...]
        #     self.obs_buf[:, 3 * i] /= self.arena_half_length
        #     self.obs_buf[:, 3 * i + 1] /= self.arena_half_width
        #     # self.obs_buf[:, 3 * i + 3:3 * i + 6] = self.robot_orientation[i, ...]
        #     # self.obs_buf[:, 3 * i + 3:3 * i + 6] /= (2 * np.pi)

        for i in range(self._totnum_agents):
            self.obs_buf[:, 4 * i:4 * i + 2] = self.robot_translation[i, :, 0:2]
            self.obs_buf[:, 4 * i] /= self.arena_half_length
            self.obs_buf[:, 4 * i + 1] /= self.arena_half_width

        for i in range(self._num_agents):
            self.obs_buf[:, 4 * i + 2] = self._distance(self.robot_position[self.num_agents], self.robot_position[i])
            self.obs_buf[:, 4 * i + 3] = torch.min(self.lidar_depth[i, ...], dim=1).values  # 形状为 (H,)
            self.obs_buf[:, 4 * i + 3] /= 20.0
        
        self.obs_buf[:, 14] = self._distance(self.robot_position[0], self.robot_position[1])
        self.obs_buf[:, 15] = self._distance(self.robot_position[0], self.robot_position[2])
        self.obs_buf[:, 16] = self._distance(self.robot_position[1], self.robot_position[2])
        ...
        if self.test:
            for j in range(self.num_envs):
                for i in range(self._totnum_agents):
                    self.trajectory_buf[j][i].append(self.obs_buf[j, 4 * i:4 * i + 2].tolist())
        return observations
    ...
    def pre_physics_step(self, actions) -> None:
        ...
        if actions == None: # pathtest
            ...
            if self.evasion_mode == 2:
                for env_id in range(self._num_envs):
                    # 当前逃逸者位置
                    evader_pos = self.robot_translation[self._num_agents, env_id, :2]

                    if evader_pos is None:
                        continue
                    
                    # 所有围捕者的位置
                    pursuer_positions = self.robot_translation[:self._num_agents, env_id, :2]

                    # 找到最近的围捕者
                    distances = torch.norm(pursuer_positions - evader_pos, dim=1)
                    closest_idx = torch.argmin(distances)
                    closest_pos = pursuer_positions[closest_idx]

                    # 计算反方向向量（逃跑方向）
                    direction = evader_pos - closest_pos
                    if torch.norm(direction) == 0:
                        direction = torch.tensor([1.0, 0.0], device=self._device)  # 如果重合，默认逃跑方向

                    direction = direction / torch.norm(direction)  # 单位向量
                    ...
                    # 计算每个方向与反方向的夹角余弦相似度（越大越接近）
                    sims = torch.mv(directions, direction)

                    # 将方向按相似度从高到低排序，优先尝试最接近反方向的
                    sorted_indices = torch.argsort(sims, descending=True)

                    found = False
                    for idx in sorted_indices:
                        candidate_direction = directions[idx]
                        for step in range(1, 20):  # 最远查找10个单位（0.5 * 20）
                            candidate_target = evader_pos + candidate_direction * (1.0 * step)

                            # 转换为 occupancy map 的栅格坐标
                            candidate_grid = parallel_pos_to_index(candidate_target.unsqueeze(0), self.occupancy_mapsize)
                            goal_coordinate = index_to_coordinate(candidate_grid[0].item(), self.occupancy_mapsize[1])

                            if judge_out_of_map(self.occupancy_map, goal_coordinate):  # 出界就跳出方向尝试
                                break
                            if not judge_obs(self.occupancy_map, goal_coordinate):  # 如果不是障碍物，就使用这个点
                                self.target_point[self._num_agents, env_id, 0] = candidate_target[0]
                                self.target_point[self._num_agents, env_id, 1] = candidate_target[1]
                                found = True
                                break
                        if found:
                            break
                        
                    # 若所有方向都无法找到合适目标点，则维持当前位置不动（可根据需要设为随机或其他处理）
                    if not found:
                        self.target_point[self._num_agents, env_id, 0] = evader_pos[0]
                        self.target_point[self._num_agents, env_id, 1] = evader_pos[1]
    ...
    def calculate_metrics(self) -> None:
        ...
        for i in range(3):
            rew += -3.0 * (1 / (self.obs_buf[:, 4 * i + 3] + 0.1))

        collision_flag = torch.ones((self._num_envs,), device=self._device, dtype=torch.bool)
        for agenti in range(0, self._num_agents):
            rew += 10 * self._is_collision(self.robot_position[self._num_agents], self.robot_position[agenti],
                                           self.robot_dist_min)
        ...
        for agenti in range(0, self._num_agents):
            ...
            # rew += -0.5 * self._distance(self.last_target_point[agenti], self.target_point[agenti])
            # rew += -0.5 * self._distance(self.target_point[agenti], self.robot_position[self._num_agents])
            rew += 5 - 0.5 * self._distance(self.robot_position[self.num_agents], self.robot_position[agenti])
            for agentj in range(agenti+1, self._num_agents):
                rew += -3 * self._is_collision(self.robot_position[agenti], self.robot_position[agentj],
                                           self.robot_dist_min)
