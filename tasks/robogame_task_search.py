# 搜救任务
# 主要改动点：get_observations、奖励函数雷达、奖励函数和is_done函数改|=和zeros、地图和csv加载路径、robogame机器人数量、self._agentname定义、pathplan函数被搜救者静止不动
...
class RoboGameTaskSearch(RLTask):
    def __init__(
            self,
            name,
            sim_config,
        ...
        self._agentname = ["red0", "red1", "blue0"]
        ...
        # 我加的部分
        self.env_name = self._cfg["env_name"] if 'env_name' in self._cfg else 'default'
        ...
        self._num_observations = 43 # 少一个agent
        self._dimension = 2
    ...
    def get_observations(self) -> dict:
        ...
        for i in range(self._num_envs):
            for j in range(self._num_agents):
                self.lidar_depth[j, i, :] = torch.from_numpy(
                    self.lidarInterface.get_linear_depth_data(f"/World/envs/env_{i}/red{j}/chassis/Lidar")[:, 0])
            
        # self.lidar_depth = torch.full_like(self.lidar_depth, 1.0)

        # print(self._totnum_agents, self._num_agents)
        ...
        self.obs_buf[:, 42] = self.progress_buf / self._max_episode_length
        # self.obs_buf[:, 60] = 0.5
    ...
    def path_plan(self):
        ...
        self.robot_vel[self._totnum_agents-1, :, :] = 0.0 # 被搜救者静止不动
    ...
    def calculate_metrics(self) -> None:
        ...
        collision_flag = torch.zeros((self._num_envs,), device=self._device, dtype=torch.bool) # 改为zeros
    ...
    def is_done(self) -> None:
        is_caught = torch.zeros((self._num_envs,), device=self._device, dtype=torch.bool) # 改为zeros
        for agenti in range(0, self._num_agents):
