...
class RoboGameTask2(RLTask):
    ...
    def get_observations(self) -> dict:
        ...
        for i in range(self._totnum_agents):
            self.obs_buf[:, 3 * i:3 * i + 2] = self.robot_translation[i, :, 0:2]
            self.obs_buf[:, 3 * i + 2] = self._distance(self.robot_position[self.num_agents], self.robot_position[i])
            self.obs_buf[:, 3 * i] /= self.arena_half_length
            self.obs_buf[:, 3 * i + 1] /= self.arena_half_width
            # self.obs_buf[:, 3 * i + 3:3 * i + 6] = self.robot_orientation[i, ...]
            # self.obs_buf[:, 3 * i + 3:3 * i + 6] /= (2 * np.pi)
        ...
        if self.test:
            for j in range(self.num_envs):
                for i in range(self._totnum_agents):
                    self.trajectory_buf[j][i].append(self.obs_buf[j, 3 * i:3 * i + 3].tolist())
        return observations
    ...
    def find_nearest_goal(self, goal_coordinate):  # find a goal not in obstacles
        visited = []
        q = queue.Queue()
        q.put(goal_coordinate)
        while q.empty() == False:
            curnode = q.get()
            visited.append(curnode)
            for i in range(-1, 2):
                for j in range(-1, 2):
                    temp = [curnode[0] + i, curnode[1] + j]
                    if judge_obs(self.occupancy_map, temp) == False:
                        return temp
                    if temp[0] >= 0 and temp[0] < self.occupancy_mapsize[0] and temp[1] >= 0 and temp[1] < \
                            self.occupancy_mapsize[1] and temp not in visited:
                        q.put(temp)
        return None
    ...
    def calculate_metrics(self) -> None:
        ...
        for i in range(3):
            rew += -torch.sum(1 / (self.lidar_depth[i, :, :] + 0.1)) / 2
        ...
        for agenti in range(0, self._num_agents):
            ...
            rew += 3 - 0.5 * self._distance(self.robot_position[self.num_agents], self.robot_position[agenti])
