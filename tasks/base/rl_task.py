...
from tasks.utils.usd_utils import create_distant_light
...
class RLTask(BaseTask):
    ...
    def cleanup(self) -> None:
        ...
        if self.test:
            self.game_record = []
            self.trajectory_buf = [[[] for j in range(self._totnum_agents)] for i in range(self._num_envs)]
            self.time_buf = torch.zeros(self._num_envs, device=self._device, dtype=torch.long)
            self.rew_record = [[] for i in range(self._num_envs)]
        self.extras = {}
