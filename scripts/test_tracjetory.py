# 正常测试，画出一次测试中最快成功围捕、最多碰撞次数、中位数情况（episode）下的各车轨迹图，选个好的用作论文展示
...
from utils.evaluation_utils.evaluation import evaluation_with_plotting
...
def parse_hydra_configs(cfg: DictConfig):
    ...
    # 读取命令行参数，如果没传就用默认值
    # 用法：/isaac-sim/python.sh scripts/test_tracjetory.py test=True +env_name=default +model_name=TD3 +checkpoint_name=TD3 +env_type=capture
    env_name = cfg.get("env_name", "default")
    model_name = cfg.get("model_name", "SAC")
    ...
    # 评估日志目录
    save_dir = os.path.join("logs", "test_tracjetory", env_type, f"{checkpoint_name}_{env_name}")
    ...
    if checkpoint_name in ALGO_MAP:
        print(f"Loading model architecture: {checkpoint_name} from {checkpoint_path}")
    ...
    evaluation_with_plotting(env, model, env_name, checkpoint_name, save_dir, n_eval_episodes=20)
